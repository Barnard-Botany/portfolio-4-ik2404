...
shannon
```
The Shannon idex is the same for all 6 samples, which makes sense because each sample has 10 unique tree species. The Shannon index gives us information regarding abundance of each species and species richness. Given that all samples have the same richness and abundance, it makes sense that the Shannon index is the same for all 6 samples. 
...
```{r}
newick_toronto = "(((((acer_negundo,acer_saccharinum,acer_platanoides)acer,ulmus_americana),(fraxinus_pennsylvanica,fraxinus_americana)fraxinus),magnolia_grandiflora)magnoliales_to_asterales,(thuja_occidentalis,(pinus_nigra,picea_glauca)pinaceae)pinales);"
tr_torontoFULL = read.tree(text=newick_toronto)
plot(tr_torontoFULL, type="cladogram", direction=c("upwards"))

par(mfrow = c(2, 2))

for (i in row.names(tt.ab)) {
plot(tr_torontoFULL, show.tip.label = FALSE, main = i)
tiplabels(tip = which(tr_torontoFULL$tip.label %in% names(which(abundance [i, ] > 0))), pch=19, cex=1)
}
```
